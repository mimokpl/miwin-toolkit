...
`miwin-uiapp` 可执行文件是**双模式**的：不带参数启动图形界面；**带命令行参数**时进入无头 CLI 模式，将桌面应用的全部能力（项目探测、数据库、后端/前端代码生成、AI 助手、远程配置）以非交互方式暴露，供开发者与 AI Agent 调用。

> 独立的 `miwin-cli` 二进制已移除——CLI 能力内置于 `miwin-uiapp` 本体。
...
```bash
cd miwin-uiapp && go build -o miwin-uiapp .
...
| 命令 | 说明 |
|------|------|
| `miwin-uiapp project inspect` | 探测项目信息（模块路径、服务列表） |
| `miwin-uiapp db test/tables/columns` | 数据库连接测试与元数据 |
| `miwin-uiapp backend grpc/rest` | 后端 Kratos 微服务代码生成 |
| `miwin-uiapp frontend gen` | 前端 CRUD 代码生成（三框架） |
| `miwin-uiapp ai presets/test/ddl/partition/review` | AI 助手 |
| `miwin-uiapp config types/services/export` | 远程配置中心导出 |
...
```bash
miwin-uiapp project inspect --path D:/GoProject/miwin-admin/backend
```
...
```bash
# 连接测试
miwin-uiapp db test --dsn "mysql://user:pass@tcp(localhost:3306)/demo"

# 列出全部表
miwin-uiapp db tables --type postgresql --host localhost --port 5432 --user demo --password demo --database demo

# 列出某表的列
miwin-uiapp db columns --dsn "..." --table sys_user
```
...
# 简写映射
miwin-uiapp backend grpc --dsn "$GOWIND_DSN" --tables user:identity,role:permission

# 只要生成产物、后处理自己控制
miwin-uiapp backend grpc --ddl schema.sql --mapping tables.json --skip-postprocess

# REST 网关（不生成 ORM/data，无后处理）
miwin-uiapp backend rest --ddl schema.sql --mapping tables.json \
...
miwin-uiapp frontend gen \
  ...
  --framework vue-vben \
  --out D:/GoProject/miwin-admin/frontend/admin/vue-vben/apps/admin/src

# 生成全部内容 JSON 到 stdout（供 AI 直接消费）
miwin-uiapp frontend gen --openapi openapi.yaml --framework react --stdout
```
...
生成器类型前缀映射（`permissionservicev1_` 等来自领域 proto 包）维护在 `miwin/pkg/frontendgen/openapi.go` 的 `serviceTypePrefixes`，黄金样本测试保证与 GUI/TS 版行为一致。
...
```bash
miwin-uiapp ai presets                          # 服务商预设
miwin-uiapp ai test                             # 连通性测试
miwin-uiapp ai ddl --requirements req.md        # 需求文档 -> MySQL DDL
miwin-uiapp ai partition --ddl schema.sql       # DDL -> 微服务划分建议
miwin-uiapp ai review --files internal/user/service.go,internal/role/service.go
```

典型的 AI 全链路组合：

```bash
miwin-uiapp ai ddl --requirements req.md --json | jq -r .content > schema.sql
miwin-uiapp ai partition --ddl schema.sql --json > partitions.json
# 依据 partitions.json 构建 mapping.json 后:
miwin-uiapp backend grpc --ddl schema.sql --mapping mapping.json
```

## config — 远程配置导出

```bash
miwin-uiapp config types
miwin-uiapp config services --path /path/to/project
miwin-uiapp config export --type consul --endpoint http://localhost:8500 \
  --project demo --path /path/to/project --dry-run
miwin-uiapp config export --type nacos --endpoint http://localhost:8848 \
