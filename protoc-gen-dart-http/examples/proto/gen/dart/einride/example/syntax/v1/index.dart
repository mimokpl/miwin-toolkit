// Code generated by protoc-gen-dart-http. DO NOT EDIT.

// ignore_for_file: unused_import, unnecessary_cast, avoid_print, non_constant_identifier_names, avoid_equals_and_hash_code_on_mutable_classes

import 'dart:async';
import 'dart:convert';
import 'dart:typed_data';
import '../../../../transport.dart';
import '../../../../proto_wire.dart';

/// Enum
enum Enum {
  enumOne('ENUM_ONE', 1),
  enumTwo('ENUM_TWO', 2),
  enumUnspecified('ENUM_UNSPECIFIED', 0);

  final String value;
  /// proto 枚举数值（二进制 wire 编解码用，与 JSON 的 value 字符串互补）
  final int wire;
  const Enum(this.value, this.wire);

  static Enum fromString(String v) =>
    values.firstWhere((e) => e.value == v, orElse: () => throw ArgumentError('Unknown Enum value: ' + v));
  static Enum fromWire(int v) =>
    values.firstWhere((e) => e.wire == v, orElse: () => throw ArgumentError('Unknown Enum wire value: ' + v.toString()));
  @override
  String toString() => value;
}

/// Message
class Message {
  /// any
  Map<String, dynamic>? any;
  /// bool
  bool? bool_;
  /// bool_value
  bool? boolValue;
  /// bytes
  String? bytes;
  /// bytes_value
  String? bytesValue;
  /// double
  double? double_;
  /// double_value
  double? doubleValue;
  /// duration
  String? duration;
  /// empty
  Map<String, dynamic>? empty;
  /// enum
  Enum? enum_;
  /// field_mask
  String? fieldMask;
  /// fixed32
  int? fixed32;
  /// fixed64
  int? fixed64;
  /// float
  double? float;
  /// float_value
  double? floatValue;
  /// int32
  int? int32;
  /// int32_value
  int? int32Value;
  /// int64
  int? int64;
  /// int64_value
  int? int64Value;
  /// list_value
  List<dynamic>? listValue;
  /// map_string_message
  Map<String, Message>? mapStringMessage;
  /// map_string_string
  Map<String, String>? mapStringString;
  /// message
  Message? message;
  /// null_value
  String? nullValue;
  /// oneof_enum
  Enum? oneofEnum;
  /// oneof_message1
  Message? oneofMessage1;
  /// oneof_message2
  Message? oneofMessage2;
  /// oneof_string
  String? oneofString;
  /// optional bool
  bool? optionalBool;
  /// optional bytes
  String? optionalBytes;
  /// optional double
  double? optionalDouble;
  /// optional enum
  Enum? optionalEnum;
  /// optional fixed32
  int? optionalFixed32;
  /// optional fixed64
  int? optionalFixed64;
  /// optional float
  double? optionalFloat;
  /// optional int32
  int? optionalInt32;
  /// optional int64
  int? optionalInt64;
  /// optional message
  Message? optionalMessage;
  /// optional sfixed32
  int? optionalSfixed32;
  /// optional sfixed64
  int? optionalSfixed64;
  /// optional sint32
  int? optionalSint32;
  /// optional sint64
  int? optionalSint64;
  /// optional string
  String? optionalString;
  /// optional uint32
  int? optionalUint32;
  /// optional uint64
  int? optionalUint64;
  /// repeated_any
  List<Map<String, dynamic>>? repeatedAny;
  /// repeated_bool
  List<bool>? repeatedBool;
  /// repeated_bool_value
  List<bool>? repeatedBoolValue;
  /// repeated_bytes
  List<String>? repeatedBytes;
  /// repeated_bytes_value
  List<String>? repeatedBytesValue;
  /// repeated_double
  List<double>? repeatedDouble;
  /// repeated_double_value
  List<double>? repeatedDoubleValue;
  /// repeated_duration
  List<String>? repeatedDuration;
  /// repeated_empty
  List<Map<String, dynamic>>? repeatedEmpty;
  /// repeated_enum
  List<Enum>? repeatedEnum;
  /// repeated_field_mask
  List<String>? repeatedFieldMask;
  /// repeated_fixed32
  List<int>? repeatedFixed32;
  /// repeated_fixed64
  List<int>? repeatedFixed64;
  /// repeated_float
  List<double>? repeatedFloat;
  /// repeated_float_value
  List<double>? repeatedFloatValue;
  /// repeated_int32
  List<int>? repeatedInt32;
  /// repeated_int32_value
  List<int>? repeatedInt32Value;
  /// repeated_int64
  List<int>? repeatedInt64;
  /// repeated_int64_value
  List<int>? repeatedInt64Value;
  /// repeated_list_value
  List<List<dynamic>>? repeatedListValue;
  /// repeated_message
  List<Message>? repeatedMessage;
  /// repeated_null_value
  List<String>? repeatedNullValue;
  /// repeated_sfixed32
  List<int>? repeatedSfixed32;
  /// repeated_sfixed64
  List<int>? repeatedSfixed64;
  /// repeated_sint32
  List<int>? repeatedSint32;
  /// repeated_sint64
  List<int>? repeatedSint64;
  /// repeated_string
  List<String>? repeatedString;
  /// repeated_string_value
  List<String>? repeatedStringValue;
  /// repeated_struct
  List<Map<String, dynamic>>? repeatedStruct;
  /// repeated_uint32
  List<int>? repeatedUint32;
  /// repeated_uint32_value
  List<int>? repeatedUint32Value;
  /// repeated_uint64
  List<int>? repeatedUint64;
  /// repeated_uint64_value
  List<int>? repeatedUint64Value;
  /// repeated_value
  List<dynamic>? repeatedValue;
  /// sfixed32
  int? sfixed32;
  /// sfixed64
  int? sfixed64;
  /// sint32
  int? sint32;
  /// sint64
  int? sint64;
  /// string
  String? string;
  /// string_value
  int? stringValue;
  /// struct
  Map<String, dynamic>? struct;
  /// uint32
  int? uint32;
  /// uint32_value
  int? uint32Value;
  /// uint64
  int? uint64;
  /// uint64_value
  int? uint64Value;
  /// value
  dynamic value;

  Message({
    this.any,
    this.bool_,
    this.boolValue,
    this.bytes,
    this.bytesValue,
    this.double_,
    this.doubleValue,
    this.duration,
    this.empty,
    this.enum_,
    this.fieldMask,
    this.fixed32,
    this.fixed64,
    this.float,
    this.floatValue,
    this.int32,
    this.int32Value,
    this.int64,
    this.int64Value,
    this.listValue,
    this.mapStringMessage,
    this.mapStringString,
    this.message,
    this.nullValue,
    this.oneofEnum,
    this.oneofMessage1,
    this.oneofMessage2,
    this.oneofString,
    this.optionalBool,
    this.optionalBytes,
    this.optionalDouble,
    this.optionalEnum,
    this.optionalFixed32,
    this.optionalFixed64,
    this.optionalFloat,
    this.optionalInt32,
    this.optionalInt64,
    this.optionalMessage,
    this.optionalSfixed32,
    this.optionalSfixed64,
    this.optionalSint32,
    this.optionalSint64,
    this.optionalString,
    this.optionalUint32,
    this.optionalUint64,
    this.repeatedAny,
    this.repeatedBool,
    this.repeatedBoolValue,
    this.repeatedBytes,
    this.repeatedBytesValue,
    this.repeatedDouble,
    this.repeatedDoubleValue,
    this.repeatedDuration,
    this.repeatedEmpty,
    this.repeatedEnum,
    this.repeatedFieldMask,
    this.repeatedFixed32,
    this.repeatedFixed64,
    this.repeatedFloat,
    this.repeatedFloatValue,
    this.repeatedInt32,
    this.repeatedInt32Value,
    this.repeatedInt64,
    this.repeatedInt64Value,
    this.repeatedListValue,
    this.repeatedMessage,
    this.repeatedNullValue,
    this.repeatedSfixed32,
    this.repeatedSfixed64,
    this.repeatedSint32,
    this.repeatedSint64,
    this.repeatedString,
    this.repeatedStringValue,
    this.repeatedStruct,
    this.repeatedUint32,
    this.repeatedUint32Value,
    this.repeatedUint64,
    this.repeatedUint64Value,
    this.repeatedValue,
    this.sfixed32,
    this.sfixed64,
    this.sint32,
    this.sint64,
    this.string,
    this.stringValue,
    this.struct,
    this.uint32,
    this.uint32Value,
    this.uint64,
    this.uint64Value,
    this.value,
  });

  factory Message.fromJson(Map<String, dynamic> json) {
    return Message(
      any: json['any'] as Map<String, dynamic>?,
      bool_: json['bool'] as bool?,
      boolValue: json['boolValue'] as bool?,
      bytes: json['bytes'] as String?,
      bytesValue: json['bytesValue'] as String?,
      double_: (json['double'] as num?)?.toDouble(),
      doubleValue: (json['doubleValue'] as num?)?.toDouble(),
      duration: json['duration'] as String?,
      empty: json['empty'] as Map<String, dynamic>?,
      enum_: json['enum'] != null ? Enum.fromString(json['enum'] as String) : null,
      fieldMask: json['fieldMask'] as String?,
      fixed32: json['fixed32'] as int?,
      fixed64: json['fixed64'] != null ? int.parse(json['fixed64'].toString()) : null,
      float: (json['float'] as num?)?.toDouble(),
      floatValue: (json['floatValue'] as num?)?.toDouble(),
      int32: json['int32'] as int?,
      int32Value: json['int32Value'] as int?,
      int64: json['int64'] != null ? int.parse(json['int64'].toString()) : null,
      int64Value: json['int64Value'] != null ? int.parse(json['int64Value'].toString()) : null,
      listValue: json['listValue'] as List<dynamic>?,
      mapStringMessage: (json['mapStringMessage'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, Message.fromJson(v as Map<String, dynamic>))),
      mapStringString: (json['mapStringString'] as Map<String, dynamic>?)?.map((k, v) => MapEntry(k, v as String)),
      message: json['message'] != null ? Message.fromJson(json['message'] as Map<String, dynamic>) : null,
      nullValue: json['nullValue'] as String?,
      oneofEnum: json['oneofEnum'] != null ? Enum.fromString(json['oneofEnum'] as String) : null,
      oneofMessage1: json['oneofMessage1'] != null ? Message.fromJson(json['oneofMessage1'] as Map<String, dynamic>) : null,
      oneofMessage2: json['oneofMessage2'] != null ? Message.fromJson(json['oneofMessage2'] as Map<String, dynamic>) : null,
      oneofString: json['oneofString'] as String?,
      optionalBool: json['optionalBool'] as bool?,
      optionalBytes: json['optionalBytes'] as String?,
      optionalDouble: (json['optionalDouble'] as num?)?.toDouble(),
      optionalEnum: json['optionalEnum'] != null ? Enum.fromString(json['optionalEnum'] as String) : null,
      optionalFixed32: json['optionalFixed32'] as int?,
      optionalFixed64: json['optionalFixed64'] != null ? int.parse(json['optionalFixed64'].toString()) : null,
      optionalFloat: (json['optionalFloat'] as num?)?.toDouble(),
      optionalInt32: json['optionalInt32'] as int?,
      optionalInt64: json['optionalInt64'] != null ? int.parse(json['optionalInt64'].toString()) : null,
      optionalMessage: json['optionalMessage'] != null ? Message.fromJson(json['optionalMessage'] as Map<String, dynamic>) : null,
      optionalSfixed32: json['optionalSfixed32'] as int?,
      optionalSfixed64: json['optionalSfixed64'] != null ? int.parse(json['optionalSfixed64'].toString()) : null,
      optionalSint32: json['optionalSint32'] as int?,
      optionalSint64: json['optionalSint64'] != null ? int.parse(json['optionalSint64'].toString()) : null,
      optionalString: json['optionalString'] as String?,
      optionalUint32: json['optionalUint32'] as int?,
      optionalUint64: json['optionalUint64'] != null ? int.parse(json['optionalUint64'].toString()) : null,
      repeatedAny: (json['repeatedAny'] as List<dynamic>?)?.map((e) => e as Map<String, dynamic>).toList(),
      repeatedBool: (json['repeatedBool'] as List<dynamic>?)?.map((e) => e as bool).toList(),
      repeatedBoolValue: (json['repeatedBoolValue'] as List<dynamic>?)?.map((e) => e as bool).toList(),
      repeatedBytes: (json['repeatedBytes'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedBytesValue: (json['repeatedBytesValue'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedDouble: (json['repeatedDouble'] as List<dynamic>?)?.map((e) => (e as num).toDouble()).toList(),
      repeatedDoubleValue: (json['repeatedDoubleValue'] as List<dynamic>?)?.map((e) => (e as num).toDouble()).toList(),
      repeatedDuration: (json['repeatedDuration'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedEmpty: (json['repeatedEmpty'] as List<dynamic>?)?.map((e) => e as Map<String, dynamic>).toList(),
      repeatedEnum: (json['repeatedEnum'] as List<dynamic>?)?.map((e) => Enum.fromString(e as String)).toList(),
      repeatedFieldMask: (json['repeatedFieldMask'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedFixed32: (json['repeatedFixed32'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedFixed64: (json['repeatedFixed64'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedFloat: (json['repeatedFloat'] as List<dynamic>?)?.map((e) => (e as num).toDouble()).toList(),
      repeatedFloatValue: (json['repeatedFloatValue'] as List<dynamic>?)?.map((e) => (e as num).toDouble()).toList(),
      repeatedInt32: (json['repeatedInt32'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedInt32Value: (json['repeatedInt32Value'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedInt64: (json['repeatedInt64'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedInt64Value: (json['repeatedInt64Value'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedListValue: (json['repeatedListValue'] as List<dynamic>?)?.map((e) => e as List<dynamic>).toList(),
      repeatedMessage: (json['repeatedMessage'] as List<dynamic>?)?.map((e) => Message.fromJson(e as Map<String, dynamic>)).toList(),
      repeatedNullValue: (json['repeatedNullValue'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedSfixed32: (json['repeatedSfixed32'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedSfixed64: (json['repeatedSfixed64'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedSint32: (json['repeatedSint32'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedSint64: (json['repeatedSint64'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedString: (json['repeatedString'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedStringValue: (json['repeatedStringValue'] as List<dynamic>?)?.map((e) => e as String).toList(),
      repeatedStruct: (json['repeatedStruct'] as List<dynamic>?)?.map((e) => e as Map<String, dynamic>).toList(),
      repeatedUint32: (json['repeatedUint32'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedUint32Value: (json['repeatedUint32Value'] as List<dynamic>?)?.map((e) => e as int).toList(),
      repeatedUint64: (json['repeatedUint64'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedUint64Value: (json['repeatedUint64Value'] as List<dynamic>?)?.map((e) => int.parse(e.toString())).toList(),
      repeatedValue: json['repeatedValue'] as List<dynamic>?,
      sfixed32: json['sfixed32'] as int?,
      sfixed64: json['sfixed64'] != null ? int.parse(json['sfixed64'].toString()) : null,
      sint32: json['sint32'] as int?,
      sint64: json['sint64'] != null ? int.parse(json['sint64'].toString()) : null,
      string: json['string'] as String?,
      stringValue: json['stringValue'] != null ? int.parse(json['stringValue'].toString()) : null,
      struct: json['struct'] as Map<String, dynamic>?,
      uint32: json['uint32'] as int?,
      uint32Value: json['uint32Value'] as int?,
      uint64: json['uint64'] != null ? int.parse(json['uint64'].toString()) : null,
      uint64Value: json['uint64Value'] != null ? int.parse(json['uint64Value'].toString()) : null,
      value: json['value'],
    );
  }

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (any != null) json['any'] = any;
    if (bool_ != null) json['bool'] = bool_;
    if (boolValue != null) json['boolValue'] = boolValue;
    if (bytes != null) json['bytes'] = bytes;
    if (bytesValue != null) json['bytesValue'] = bytesValue;
    if (double_ != null) json['double'] = double_;
    if (doubleValue != null) json['doubleValue'] = doubleValue;
    if (duration != null) json['duration'] = duration;
    if (empty != null) json['empty'] = empty;
    if (enum_ != null) json['enum'] = enum_!.value;
    if (fieldMask != null) json['fieldMask'] = fieldMask;
    if (fixed32 != null) json['fixed32'] = fixed32;
    if (fixed64 != null) json['fixed64'] = fixed64.toString();
    if (float != null) json['float'] = float;
    if (floatValue != null) json['floatValue'] = floatValue;
    if (int32 != null) json['int32'] = int32;
    if (int32Value != null) json['int32Value'] = int32Value;
    if (int64 != null) json['int64'] = int64.toString();
    if (int64Value != null) json['int64Value'] = int64Value.toString();
    if (listValue != null) json['listValue'] = listValue;
    if (mapStringMessage != null) json['mapStringMessage'] = mapStringMessage!.map((k, v) => MapEntry(k, v.toJson()));
    if (mapStringString != null) json['mapStringString'] = mapStringString;
    if (message != null) json['message'] = message!.toJson();
    if (nullValue != null) json['nullValue'] = nullValue;
    if (oneofEnum != null) json['oneofEnum'] = oneofEnum!.value;
    if (oneofMessage1 != null) json['oneofMessage1'] = oneofMessage1!.toJson();
    if (oneofMessage2 != null) json['oneofMessage2'] = oneofMessage2!.toJson();
    if (oneofString != null) json['oneofString'] = oneofString;
    if (optionalBool != null) json['optionalBool'] = optionalBool;
    if (optionalBytes != null) json['optionalBytes'] = optionalBytes;
    if (optionalDouble != null) json['optionalDouble'] = optionalDouble;
    if (optionalEnum != null) json['optionalEnum'] = optionalEnum!.value;
    if (optionalFixed32 != null) json['optionalFixed32'] = optionalFixed32;
    if (optionalFixed64 != null) json['optionalFixed64'] = optionalFixed64.toString();
    if (optionalFloat != null) json['optionalFloat'] = optionalFloat;
    if (optionalInt32 != null) json['optionalInt32'] = optionalInt32;
    if (optionalInt64 != null) json['optionalInt64'] = optionalInt64.toString();
    if (optionalMessage != null) json['optionalMessage'] = optionalMessage!.toJson();
    if (optionalSfixed32 != null) json['optionalSfixed32'] = optionalSfixed32;
    if (optionalSfixed64 != null) json['optionalSfixed64'] = optionalSfixed64.toString();
    if (optionalSint32 != null) json['optionalSint32'] = optionalSint32;
    if (optionalSint64 != null) json['optionalSint64'] = optionalSint64.toString();
    if (optionalString != null) json['optionalString'] = optionalString;
    if (optionalUint32 != null) json['optionalUint32'] = optionalUint32;
    if (optionalUint64 != null) json['optionalUint64'] = optionalUint64.toString();
    if (repeatedAny != null) json['repeatedAny'] = repeatedAny;
    if (repeatedBool != null) json['repeatedBool'] = repeatedBool;
    if (repeatedBoolValue != null) json['repeatedBoolValue'] = repeatedBoolValue;
    if (repeatedBytes != null) json['repeatedBytes'] = repeatedBytes;
    if (repeatedBytesValue != null) json['repeatedBytesValue'] = repeatedBytesValue;
    if (repeatedDouble != null) json['repeatedDouble'] = repeatedDouble;
    if (repeatedDoubleValue != null) json['repeatedDoubleValue'] = repeatedDoubleValue;
    if (repeatedDuration != null) json['repeatedDuration'] = repeatedDuration;
    if (repeatedEmpty != null) json['repeatedEmpty'] = repeatedEmpty;
    if (repeatedEnum != null) json['repeatedEnum'] = repeatedEnum!.map((e) => e.value).toList();
    if (repeatedFieldMask != null) json['repeatedFieldMask'] = repeatedFieldMask;
    if (repeatedFixed32 != null) json['repeatedFixed32'] = repeatedFixed32;
    if (repeatedFixed64 != null) json['repeatedFixed64'] = repeatedFixed64!.map((e) => e.toString()).toList();
    if (repeatedFloat != null) json['repeatedFloat'] = repeatedFloat;
    if (repeatedFloatValue != null) json['repeatedFloatValue'] = repeatedFloatValue;
    if (repeatedInt32 != null) json['repeatedInt32'] = repeatedInt32;
    if (repeatedInt32Value != null) json['repeatedInt32Value'] = repeatedInt32Value;
    if (repeatedInt64 != null) json['repeatedInt64'] = repeatedInt64!.map((e) => e.toString()).toList();
    if (repeatedInt64Value != null) json['repeatedInt64Value'] = repeatedInt64Value!.map((e) => e.toString()).toList();
    if (repeatedListValue != null) json['repeatedListValue'] = repeatedListValue;
    if (repeatedMessage != null) json['repeatedMessage'] = repeatedMessage!.map((e) => e.toJson()).toList();
    if (repeatedNullValue != null) json['repeatedNullValue'] = repeatedNullValue;
    if (repeatedSfixed32 != null) json['repeatedSfixed32'] = repeatedSfixed32;
    if (repeatedSfixed64 != null) json['repeatedSfixed64'] = repeatedSfixed64!.map((e) => e.toString()).toList();
    if (repeatedSint32 != null) json['repeatedSint32'] = repeatedSint32;
    if (repeatedSint64 != null) json['repeatedSint64'] = repeatedSint64!.map((e) => e.toString()).toList();
    if (repeatedString != null) json['repeatedString'] = repeatedString;
    if (repeatedStringValue != null) json['repeatedStringValue'] = repeatedStringValue;
    if (repeatedStruct != null) json['repeatedStruct'] = repeatedStruct;
    if (repeatedUint32 != null) json['repeatedUint32'] = repeatedUint32;
    if (repeatedUint32Value != null) json['repeatedUint32Value'] = repeatedUint32Value;
    if (repeatedUint64 != null) json['repeatedUint64'] = repeatedUint64!.map((e) => e.toString()).toList();
    if (repeatedUint64Value != null) json['repeatedUint64Value'] = repeatedUint64Value!.map((e) => e.toString()).toList();
    if (repeatedValue != null) json['repeatedValue'] = repeatedValue;
    if (sfixed32 != null) json['sfixed32'] = sfixed32;
    if (sfixed64 != null) json['sfixed64'] = sfixed64.toString();
    if (sint32 != null) json['sint32'] = sint32;
    if (sint64 != null) json['sint64'] = sint64.toString();
    if (string != null) json['string'] = string;
    if (stringValue != null) json['stringValue'] = stringValue.toString();
    if (struct != null) json['struct'] = struct;
    if (uint32 != null) json['uint32'] = uint32;
    if (uint32Value != null) json['uint32Value'] = uint32Value;
    if (uint64 != null) json['uint64'] = uint64.toString();
    if (uint64Value != null) json['uint64Value'] = uint64Value.toString();
    if (value != null) json['value'] = value;
    return json;
  }

  @override
  String toString() {
    return 'Message(any: $any, bool_: $bool_, boolValue: $boolValue, bytes: $bytes, bytesValue: $bytesValue, double_: $double_, doubleValue: $doubleValue, duration: $duration, empty: $empty, enum_: $enum_, fieldMask: $fieldMask, fixed32: $fixed32, fixed64: $fixed64, float: $float, floatValue: $floatValue, int32: $int32, int32Value: $int32Value, int64: $int64, int64Value: $int64Value, listValue: $listValue, mapStringMessage: $mapStringMessage, mapStringString: $mapStringString, message: $message, nullValue: $nullValue, oneofEnum: $oneofEnum, oneofMessage1: $oneofMessage1, oneofMessage2: $oneofMessage2, oneofString: $oneofString, optionalBool: $optionalBool, optionalBytes: $optionalBytes, optionalDouble: $optionalDouble, optionalEnum: $optionalEnum, optionalFixed32: $optionalFixed32, optionalFixed64: $optionalFixed64, optionalFloat: $optionalFloat, optionalInt32: $optionalInt32, optionalInt64: $optionalInt64, optionalMessage: $optionalMessage, optionalSfixed32: $optionalSfixed32, optionalSfixed64: $optionalSfixed64, optionalSint32: $optionalSint32, optionalSint64: $optionalSint64, optionalString: $optionalString, optionalUint32: $optionalUint32, optionalUint64: $optionalUint64, repeatedAny: $repeatedAny, repeatedBool: $repeatedBool, repeatedBoolValue: $repeatedBoolValue, repeatedBytes: $repeatedBytes, repeatedBytesValue: $repeatedBytesValue, repeatedDouble: $repeatedDouble, repeatedDoubleValue: $repeatedDoubleValue, repeatedDuration: $repeatedDuration, repeatedEmpty: $repeatedEmpty, repeatedEnum: $repeatedEnum, repeatedFieldMask: $repeatedFieldMask, repeatedFixed32: $repeatedFixed32, repeatedFixed64: $repeatedFixed64, repeatedFloat: $repeatedFloat, repeatedFloatValue: $repeatedFloatValue, repeatedInt32: $repeatedInt32, repeatedInt32Value: $repeatedInt32Value, repeatedInt64: $repeatedInt64, repeatedInt64Value: $repeatedInt64Value, repeatedListValue: $repeatedListValue, repeatedMessage: $repeatedMessage, repeatedNullValue: $repeatedNullValue, repeatedSfixed32: $repeatedSfixed32, repeatedSfixed64: $repeatedSfixed64, repeatedSint32: $repeatedSint32, repeatedSint64: $repeatedSint64, repeatedString: $repeatedString, repeatedStringValue: $repeatedStringValue, repeatedStruct: $repeatedStruct, repeatedUint32: $repeatedUint32, repeatedUint32Value: $repeatedUint32Value, repeatedUint64: $repeatedUint64, repeatedUint64Value: $repeatedUint64Value, repeatedValue: $repeatedValue, sfixed32: $sfixed32, sfixed64: $sfixed64, sint32: $sint32, sint64: $sint64, string: $string, stringValue: $stringValue, struct: $struct, uint32: $uint32, uint32Value: $uint32Value, uint64: $uint64, uint64Value: $uint64Value, value: $value)';
  }

  @override
  bool operator ==(Object other) =>
    identical(this, other) ||
    other is Message &&
      runtimeType == other.runtimeType
      && any == other.any
      && bool_ == other.bool_
      && boolValue == other.boolValue
      && bytes == other.bytes
      && bytesValue == other.bytesValue
      && double_ == other.double_
      && doubleValue == other.doubleValue
      && duration == other.duration
      && empty == other.empty
      && enum_ == other.enum_
      && fieldMask == other.fieldMask
      && fixed32 == other.fixed32
      && fixed64 == other.fixed64
      && float == other.float
      && floatValue == other.floatValue
      && int32 == other.int32
      && int32Value == other.int32Value
      && int64 == other.int64
      && int64Value == other.int64Value
      && listValue == other.listValue
      && mapStringMessage == other.mapStringMessage
      && mapStringString == other.mapStringString
      && message == other.message
      && nullValue == other.nullValue
      && oneofEnum == other.oneofEnum
      && oneofMessage1 == other.oneofMessage1
      && oneofMessage2 == other.oneofMessage2
      && oneofString == other.oneofString
      && optionalBool == other.optionalBool
      && optionalBytes == other.optionalBytes
      && optionalDouble == other.optionalDouble
      && optionalEnum == other.optionalEnum
      && optionalFixed32 == other.optionalFixed32
      && optionalFixed64 == other.optionalFixed64
      && optionalFloat == other.optionalFloat
      && optionalInt32 == other.optionalInt32
      && optionalInt64 == other.optionalInt64
      && optionalMessage == other.optionalMessage
      && optionalSfixed32 == other.optionalSfixed32
      && optionalSfixed64 == other.optionalSfixed64
      && optionalSint32 == other.optionalSint32
      && optionalSint64 == other.optionalSint64
      && optionalString == other.optionalString
      && optionalUint32 == other.optionalUint32
      && optionalUint64 == other.optionalUint64
      && repeatedAny == other.repeatedAny
      && repeatedBool == other.repeatedBool
      && repeatedBoolValue == other.repeatedBoolValue
      && repeatedBytes == other.repeatedBytes
      && repeatedBytesValue == other.repeatedBytesValue
      && repeatedDouble == other.repeatedDouble
      && repeatedDoubleValue == other.repeatedDoubleValue
      && repeatedDuration == other.repeatedDuration
      && repeatedEmpty == other.repeatedEmpty
      && repeatedEnum == other.repeatedEnum
      && repeatedFieldMask == other.repeatedFieldMask
      && repeatedFixed32 == other.repeatedFixed32
      && repeatedFixed64 == other.repeatedFixed64
      && repeatedFloat == other.repeatedFloat
      && repeatedFloatValue == other.repeatedFloatValue
      && repeatedInt32 == other.repeatedInt32
      && repeatedInt32Value == other.repeatedInt32Value
      && repeatedInt64 == other.repeatedInt64
      && repeatedInt64Value == other.repeatedInt64Value
      && repeatedListValue == other.repeatedListValue
      && repeatedMessage == other.repeatedMessage
      && repeatedNullValue == other.repeatedNullValue
      && repeatedSfixed32 == other.repeatedSfixed32
      && repeatedSfixed64 == other.repeatedSfixed64
      && repeatedSint32 == other.repeatedSint32
      && repeatedSint64 == other.repeatedSint64
      && repeatedString == other.repeatedString
      && repeatedStringValue == other.repeatedStringValue
      && repeatedStruct == other.repeatedStruct
      && repeatedUint32 == other.repeatedUint32
      && repeatedUint32Value == other.repeatedUint32Value
      && repeatedUint64 == other.repeatedUint64
      && repeatedUint64Value == other.repeatedUint64Value
      && repeatedValue == other.repeatedValue
      && sfixed32 == other.sfixed32
      && sfixed64 == other.sfixed64
      && sint32 == other.sint32
      && sint64 == other.sint64
      && string == other.string
      && stringValue == other.stringValue
      && struct == other.struct
      && uint32 == other.uint32
      && uint32Value == other.uint32Value
      && uint64 == other.uint64
      && uint64Value == other.uint64Value
      && value == other.value
    ;

  @override
  int get hashCode => Object.hashAll([
    any,
    bool_,
    boolValue,
    bytes,
    bytesValue,
    double_,
    doubleValue,
    duration,
    empty,
    enum_,
    fieldMask,
    fixed32,
    fixed64,
    float,
    floatValue,
    int32,
    int32Value,
    int64,
    int64Value,
    listValue,
    mapStringMessage,
    mapStringString,
    message,
    nullValue,
    oneofEnum,
    oneofMessage1,
    oneofMessage2,
    oneofString,
    optionalBool,
    optionalBytes,
    optionalDouble,
    optionalEnum,
    optionalFixed32,
    optionalFixed64,
    optionalFloat,
    optionalInt32,
    optionalInt64,
    optionalMessage,
    optionalSfixed32,
    optionalSfixed64,
    optionalSint32,
    optionalSint64,
    optionalString,
    optionalUint32,
    optionalUint64,
    repeatedAny,
    repeatedBool,
    repeatedBoolValue,
    repeatedBytes,
    repeatedBytesValue,
    repeatedDouble,
    repeatedDoubleValue,
    repeatedDuration,
    repeatedEmpty,
    repeatedEnum,
    repeatedFieldMask,
    repeatedFixed32,
    repeatedFixed64,
    repeatedFloat,
    repeatedFloatValue,
    repeatedInt32,
    repeatedInt32Value,
    repeatedInt64,
    repeatedInt64Value,
    repeatedListValue,
    repeatedMessage,
    repeatedNullValue,
    repeatedSfixed32,
    repeatedSfixed64,
    repeatedSint32,
    repeatedSint64,
    repeatedString,
    repeatedStringValue,
    repeatedStruct,
    repeatedUint32,
    repeatedUint32Value,
    repeatedUint64,
    repeatedUint64Value,
    repeatedValue,
    sfixed32,
    sfixed64,
    sint32,
    sint64,
    string,
    stringValue,
    struct,
    uint32,
    uint32Value,
    uint64,
    uint64Value,
    value,
  ]);

  Message copyWith({
    Map<String, dynamic>? any,
    bool? bool_,
    bool? boolValue,
    String? bytes,
    String? bytesValue,
    double? double_,
    double? doubleValue,
    String? duration,
    Map<String, dynamic>? empty,
    Enum? enum_,
    String? fieldMask,
    int? fixed32,
    int? fixed64,
    double? float,
    double? floatValue,
    int? int32,
    int? int32Value,
    int? int64,
    int? int64Value,
    List<dynamic>? listValue,
    Map<String, Message>? mapStringMessage,
    Map<String, String>? mapStringString,
    Message? message,
    String? nullValue,
    Enum? oneofEnum,
    Message? oneofMessage1,
    Message? oneofMessage2,
    String? oneofString,
    bool? optionalBool,
    String? optionalBytes,
    double? optionalDouble,
    Enum? optionalEnum,
    int? optionalFixed32,
    int? optionalFixed64,
    double? optionalFloat,
    int? optionalInt32,
    int? optionalInt64,
    Message? optionalMessage,
    int? optionalSfixed32,
    int? optionalSfixed64,
    int? optionalSint32,
    int? optionalSint64,
    String? optionalString,
    int? optionalUint32,
    int? optionalUint64,
    List<Map<String, dynamic>>? repeatedAny,
    List<bool>? repeatedBool,
    List<bool>? repeatedBoolValue,
    List<String>? repeatedBytes,
    List<String>? repeatedBytesValue,
    List<double>? repeatedDouble,
    List<double>? repeatedDoubleValue,
    List<String>? repeatedDuration,
    List<Map<String, dynamic>>? repeatedEmpty,
    List<Enum>? repeatedEnum,
    List<String>? repeatedFieldMask,
    List<int>? repeatedFixed32,
    List<int>? repeatedFixed64,
    List<double>? repeatedFloat,
    List<double>? repeatedFloatValue,
    List<int>? repeatedInt32,
    List<int>? repeatedInt32Value,
    List<int>? repeatedInt64,
    List<int>? repeatedInt64Value,
    List<List<dynamic>>? repeatedListValue,
    List<Message>? repeatedMessage,
    List<String>? repeatedNullValue,
    List<int>? repeatedSfixed32,
    List<int>? repeatedSfixed64,
    List<int>? repeatedSint32,
    List<int>? repeatedSint64,
    List<String>? repeatedString,
    List<String>? repeatedStringValue,
    List<Map<String, dynamic>>? repeatedStruct,
    List<int>? repeatedUint32,
    List<int>? repeatedUint32Value,
    List<int>? repeatedUint64,
    List<int>? repeatedUint64Value,
    List<dynamic>? repeatedValue,
    int? sfixed32,
    int? sfixed64,
    int? sint32,
    int? sint64,
    String? string,
    int? stringValue,
    Map<String, dynamic>? struct,
    int? uint32,
    int? uint32Value,
    int? uint64,
    int? uint64Value,
    dynamic value,
  }) {
    return Message(
      any: any ?? this.any,
      bool_: bool_ ?? this.bool_,
      boolValue: boolValue ?? this.boolValue,
      bytes: bytes ?? this.bytes,
      bytesValue: bytesValue ?? this.bytesValue,
      double_: double_ ?? this.double_,
      doubleValue: doubleValue ?? this.doubleValue,
      duration: duration ?? this.duration,
      empty: empty ?? this.empty,
      enum_: enum_ ?? this.enum_,
      fieldMask: fieldMask ?? this.fieldMask,
      fixed32: fixed32 ?? this.fixed32,
      fixed64: fixed64 ?? this.fixed64,
      float: float ?? this.float,
      floatValue: floatValue ?? this.floatValue,
      int32: int32 ?? this.int32,
      int32Value: int32Value ?? this.int32Value,
      int64: int64 ?? this.int64,
      int64Value: int64Value ?? this.int64Value,
      listValue: listValue ?? this.listValue,
      mapStringMessage: mapStringMessage ?? this.mapStringMessage,
      mapStringString: mapStringString ?? this.mapStringString,
      message: message ?? this.message,
      nullValue: nullValue ?? this.nullValue,
      oneofEnum: oneofEnum ?? this.oneofEnum,
      oneofMessage1: oneofMessage1 ?? this.oneofMessage1,
      oneofMessage2: oneofMessage2 ?? this.oneofMessage2,
      oneofString: oneofString ?? this.oneofString,
      optionalBool: optionalBool ?? this.optionalBool,
      optionalBytes: optionalBytes ?? this.optionalBytes,
      optionalDouble: optionalDouble ?? this.optionalDouble,
      optionalEnum: optionalEnum ?? this.optionalEnum,
      optionalFixed32: optionalFixed32 ?? this.optionalFixed32,
      optionalFixed64: optionalFixed64 ?? this.optionalFixed64,
      optionalFloat: optionalFloat ?? this.optionalFloat,
      optionalInt32: optionalInt32 ?? this.optionalInt32,
      optionalInt64: optionalInt64 ?? this.optionalInt64,
      optionalMessage: optionalMessage ?? this.optionalMessage,
      optionalSfixed32: optionalSfixed32 ?? this.optionalSfixed32,
      optionalSfixed64: optionalSfixed64 ?? this.optionalSfixed64,
      optionalSint32: optionalSint32 ?? this.optionalSint32,
      optionalSint64: optionalSint64 ?? this.optionalSint64,
      optionalString: optionalString ?? this.optionalString,
      optionalUint32: optionalUint32 ?? this.optionalUint32,
      optionalUint64: optionalUint64 ?? this.optionalUint64,
      repeatedAny: repeatedAny ?? this.repeatedAny,
      repeatedBool: repeatedBool ?? this.repeatedBool,
      repeatedBoolValue: repeatedBoolValue ?? this.repeatedBoolValue,
      repeatedBytes: repeatedBytes ?? this.repeatedBytes,
      repeatedBytesValue: repeatedBytesValue ?? this.repeatedBytesValue,
      repeatedDouble: repeatedDouble ?? this.repeatedDouble,
      repeatedDoubleValue: repeatedDoubleValue ?? this.repeatedDoubleValue,
      repeatedDuration: repeatedDuration ?? this.repeatedDuration,
      repeatedEmpty: repeatedEmpty ?? this.repeatedEmpty,
      repeatedEnum: repeatedEnum ?? this.repeatedEnum,
      repeatedFieldMask: repeatedFieldMask ?? this.repeatedFieldMask,
      repeatedFixed32: repeatedFixed32 ?? this.repeatedFixed32,
      repeatedFixed64: repeatedFixed64 ?? this.repeatedFixed64,
      repeatedFloat: repeatedFloat ?? this.repeatedFloat,
      repeatedFloatValue: repeatedFloatValue ?? this.repeatedFloatValue,
      repeatedInt32: repeatedInt32 ?? this.repeatedInt32,
      repeatedInt32Value: repeatedInt32Value ?? this.repeatedInt32Value,
      repeatedInt64: repeatedInt64 ?? this.repeatedInt64,
      repeatedInt64Value: repeatedInt64Value ?? this.repeatedInt64Value,
      repeatedListValue: repeatedListValue ?? this.repeatedListValue,
      repeatedMessage: repeatedMessage ?? this.repeatedMessage,
      repeatedNullValue: repeatedNullValue ?? this.repeatedNullValue,
      repeatedSfixed32: repeatedSfixed32 ?? this.repeatedSfixed32,
      repeatedSfixed64: repeatedSfixed64 ?? this.repeatedSfixed64,
      repeatedSint32: repeatedSint32 ?? this.repeatedSint32,
      repeatedSint64: repeatedSint64 ?? this.repeatedSint64,
      repeatedString: repeatedString ?? this.repeatedString,
      repeatedStringValue: repeatedStringValue ?? this.repeatedStringValue,
      repeatedStruct: repeatedStruct ?? this.repeatedStruct,
      repeatedUint32: repeatedUint32 ?? this.repeatedUint32,
      repeatedUint32Value: repeatedUint32Value ?? this.repeatedUint32Value,
      repeatedUint64: repeatedUint64 ?? this.repeatedUint64,
      repeatedUint64Value: repeatedUint64Value ?? this.repeatedUint64Value,
      repeatedValue: repeatedValue ?? this.repeatedValue,
      sfixed32: sfixed32 ?? this.sfixed32,
      sfixed64: sfixed64 ?? this.sfixed64,
      sint32: sint32 ?? this.sint32,
      sint64: sint64 ?? this.sint64,
      string: string ?? this.string,
      stringValue: stringValue ?? this.stringValue,
      struct: struct ?? this.struct,
      uint32: uint32 ?? this.uint32,
      uint32Value: uint32Value ?? this.uint32Value,
      uint64: uint64 ?? this.uint64,
      uint64Value: uint64Value ?? this.uint64Value,
      value: value ?? this.value,
    );
  }

  /// 二进制 proto 编码（MQTT 等二进制传输用；字段按编号升序写出）
  Uint8List writeToBuffer() {
    final w = ProtoWireWriter();
    _writeTo(w);
    return w.toBuffer();
  }

  void _writeTo(ProtoWireWriter w) {
    // field 1: double
    final f1 = double_;
    if (f1 != null) {
      w.writeDouble(1, f1);
    }
    // field 2: float
    final f2 = float;
    if (f2 != null) {
      w.writeFloat(2, f2);
    }
    // field 3: int32
    final f3 = int32;
    if (f3 != null) {
      w.writeInt32(3, f3);
    }
    // field 4: int64
    final f4 = int64;
    if (f4 != null) {
      w.writeInt64(4, f4);
    }
    // field 5: uint32
    final f5 = uint32;
    if (f5 != null) {
      w.writeUint32(5, f5);
    }
    // field 6: uint64
    final f6 = uint64;
    if (f6 != null) {
      w.writeUint64(6, f6);
    }
    // field 7: sint32
    final f7 = sint32;
    if (f7 != null) {
      w.writeSint32(7, f7);
    }
    // field 8: sint64
    final f8 = sint64;
    if (f8 != null) {
      w.writeSint64(8, f8);
    }
    // field 9: fixed32
    final f9 = fixed32;
    if (f9 != null) {
      w.writeFixed32(9, f9);
    }
    // field 10: fixed64
    final f10 = fixed64;
    if (f10 != null) {
      w.writeFixed64(10, f10);
    }
    // field 11: sfixed32
    final f11 = sfixed32;
    if (f11 != null) {
      w.writeSfixed32(11, f11);
    }
    // field 12: sfixed64
    final f12 = sfixed64;
    if (f12 != null) {
      w.writeSfixed64(12, f12);
    }
    // field 13: bool
    final f13 = bool_;
    if (f13 != null) {
      w.writeBool(13, f13);
    }
    // field 14: string
    final f14 = string;
    if (f14 != null) {
      w.writeString(14, f14);
    }
    // field 15: bytes
    final f15 = bytes;
    if (f15 != null) {
      throw UnsupportedError('dart-http wire: field einride.example.syntax.v1.Message.bytes');
    }
    // field 16: enum
    final f16 = enum_;
    if (f16 != null) {
      w.writeEnum(16, f16.wire); // Enum
    }
    // field 17: message
    final f17 = message;
    if (f17 != null) {
      final cw = ProtoWireWriter(); f17._writeTo(cw); w.writeRaw(17, cw.toBuffer()); // Message
    }
    // field 18: repeatedDouble
    final f18 = repeatedDouble;
    if (f18 != null) {
      w.writePackedDoubleList(18, f18);
    }
    // field 19: repeatedFloat
    final f19 = repeatedFloat;
    if (f19 != null) {
      w.writePackedFloatList(19, f19);
    }
    // field 20: repeatedInt32
    final f20 = repeatedInt32;
    if (f20 != null) {
      w.writePackedVarintList(20, f20);
    }
    // field 21: repeatedInt64
    final f21 = repeatedInt64;
    if (f21 != null) {
      w.writePackedVarintList(21, f21);
    }
    // field 22: repeatedUint32
    final f22 = repeatedUint32;
    if (f22 != null) {
      w.writePackedVarintList(22, f22);
    }
    // field 23: repeatedUint64
    final f23 = repeatedUint64;
    if (f23 != null) {
      w.writePackedVarintList(23, f23);
    }
    // field 24: repeatedSint32
    final f24 = repeatedSint32;
    if (f24 != null) {
      w.writePackedSint32List(24, f24);
    }
    // field 25: repeatedSint64
    final f25 = repeatedSint64;
    if (f25 != null) {
      w.writePackedSint64List(25, f25);
    }
    // field 26: repeatedFixed32
    final f26 = repeatedFixed32;
    if (f26 != null) {
      w.writePackedFixed32List(26, f26);
    }
    // field 27: repeatedFixed64
    final f27 = repeatedFixed64;
    if (f27 != null) {
      w.writePackedFixed64List(27, f27);
    }
    // field 28: repeatedSfixed32
    final f28 = repeatedSfixed32;
    if (f28 != null) {
      w.writePackedFixed32List(28, f28);
    }
    // field 29: repeatedSfixed64
    final f29 = repeatedSfixed64;
    if (f29 != null) {
      w.writePackedFixed64List(29, f29);
    }
    // field 30: repeatedBool
    final f30 = repeatedBool;
    if (f30 != null) {
      w.writePackedBoolList(30, f30);
    }
    // field 31: repeatedString
    final f31 = repeatedString;
    if (f31 != null) {
      w.writeStringList(31, f31);
    }
    // field 32: repeatedBytes
    final f32 = repeatedBytes;
    if (f32 != null) {
      w.writeBytesList(32, f32);
    }
    // field 33: repeatedEnum
    final f33 = repeatedEnum;
    if (f33 != null) {
      w.writePackedVarintList(33, f33.map((e) => e.wire).toList()); // Enum
    }
    // field 34: repeatedMessage
    final f34 = repeatedMessage;
    if (f34 != null) {
      for (final e in f34) { final cw = ProtoWireWriter(); e._writeTo(cw); w.writeRaw(34, cw.toBuffer()); }
    }
    // field 35: mapStringString
    final f35 = mapStringString;
    if (f35 != null) {
      f35.forEach((k, v) {
        final ew = ProtoWireWriter();
        ew.writeString(1, k);
        ew.writeString(2, v);
        w.writeRaw(35, ew.toBuffer());
      });
    }
    // field 36: mapStringMessage
    final f36 = mapStringMessage;
    if (f36 != null) {
      f36.forEach((k, v) {
        final ew = ProtoWireWriter();
        ew.writeString(1, k);
        final vw = ProtoWireWriter(); v._writeTo(vw); ew.writeRaw(2, vw.toBuffer());
        w.writeRaw(36, ew.toBuffer());
      });
    }
    // field 37: oneofString
    final f37 = oneofString;
    if (f37 != null) {
      w.writeString(37, f37);
    }
    // field 38: oneofEnum
    final f38 = oneofEnum;
    if (f38 != null) {
      w.writeEnum(38, f38.wire); // Enum
    }
    // field 39: oneofMessage1
    final f39 = oneofMessage1;
    if (f39 != null) {
      final cw = ProtoWireWriter(); f39._writeTo(cw); w.writeRaw(39, cw.toBuffer()); // Message
    }
    // field 40: oneofMessage2
    final f40 = oneofMessage2;
    if (f40 != null) {
      final cw = ProtoWireWriter(); f40._writeTo(cw); w.writeRaw(40, cw.toBuffer()); // Message
    }
    // field 41: any
    final f41 = any;
    if (f41 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Any not supported');
    }
    // field 42: repeatedAny
    final f42 = repeatedAny;
    if (f42 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Any not supported');
    }
    // field 43: duration
    final f43 = duration;
    if (f43 != null) {
      w.writeDuration(43, f43);
    }
    // field 44: repeatedDuration
    final f44 = repeatedDuration;
    if (f44 != null) {
      for (final e in f44) { w.writeDuration(44, e); }
    }
    // field 45: empty
    final f45 = empty;
    if (f45 != null) {
      w.writeEmpty(45);
    }
    // field 46: repeatedEmpty
    final f46 = repeatedEmpty;
    if (f46 != null) {
      for (final _ in f46) { w.writeEmpty(46); }
    }
    // field 47: fieldMask
    final f47 = fieldMask;
    if (f47 != null) {
      w.writeFieldMask(47, f47);
    }
    // field 48: repeatedFieldMask
    final f48 = repeatedFieldMask;
    if (f48 != null) {
      for (final e in f48) { w.writeFieldMask(48, e); }
    }
    // field 49: struct
    final f49 = struct;
    if (f49 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Struct not supported');
    }
    // field 50: repeatedStruct
    final f50 = repeatedStruct;
    if (f50 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Struct not supported');
    }
    // field 51: value
    final f51 = value;
    if (f51 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Value not supported');
    }
    // field 52: repeatedValue
    final f52 = repeatedValue;
    if (f52 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.Value not supported');
    }
    // field 53: nullValue
    final f53 = nullValue;
    if (f53 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.NullValue not supported');
    }
    // field 54: repeatedNullValue
    final f54 = repeatedNullValue;
    if (f54 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.NullValue not supported');
    }
    // field 55: listValue
    final f55 = listValue;
    if (f55 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.ListValue not supported');
    }
    // field 56: repeatedListValue
    final f56 = repeatedListValue;
    if (f56 != null) {
      throw UnsupportedError('dart-http wire: google.protobuf.ListValue not supported');
    }
    // field 57: boolValue
    final f57 = boolValue;
    if (f57 != null) {
      w.writeBoolValue(57, f57);
    }
    // field 58: repeatedBoolValue
    final f58 = repeatedBoolValue;
    if (f58 != null) {
      for (final e in f58) { w.writeBoolValue(58, e); }
    }
    // field 59: bytesValue
    final f59 = bytesValue;
    if (f59 != null) {
      w.writeBytesValue(59, f59);
    }
    // field 60: repeatedBytesValue
    final f60 = repeatedBytesValue;
    if (f60 != null) {
      for (final e in f60) { w.writeBytesValue(60, e); }
    }
    // field 61: doubleValue
    final f61 = doubleValue;
    if (f61 != null) {
      w.writeDoubleValue(61, f61);
    }
    // field 62: repeatedDoubleValue
    final f62 = repeatedDoubleValue;
    if (f62 != null) {
      for (final e in f62) { w.writeDoubleValue(62, e); }
    }
    // field 63: floatValue
    final f63 = floatValue;
    if (f63 != null) {
      w.writeFloatValue(63, f63);
    }
    // field 64: repeatedFloatValue
    final f64 = repeatedFloatValue;
    if (f64 != null) {
      for (final e in f64) { w.writeFloatValue(64, e); }
    }
    // field 65: int32Value
    final f65 = int32Value;
    if (f65 != null) {
      w.writeInt32Value(65, f65);
    }
    // field 66: repeatedInt32Value
    final f66 = repeatedInt32Value;
    if (f66 != null) {
      for (final e in f66) { w.writeInt32Value(66, e); }
    }
    // field 67: int64Value
    final f67 = int64Value;
    if (f67 != null) {
      w.writeInt64Value(67, f67);
    }
    // field 68: repeatedInt64Value
    final f68 = repeatedInt64Value;
    if (f68 != null) {
      for (final e in f68) { w.writeInt64Value(68, e); }
    }
    // field 69: uint32Value
    final f69 = uint32Value;
    if (f69 != null) {
      w.writeUint32Value(69, f69);
    }
    // field 70: repeatedUint32Value
    final f70 = repeatedUint32Value;
    if (f70 != null) {
      for (final e in f70) { w.writeUint32Value(70, e); }
    }
    // field 71: uint64Value
    final f71 = uint64Value;
    if (f71 != null) {
      w.writeUint64Value(71, f71);
    }
    // field 72: repeatedUint64Value
    final f72 = repeatedUint64Value;
    if (f72 != null) {
      for (final e in f72) { w.writeUint64Value(72, e); }
    }
    // field 73: stringValue
    final f73 = stringValue;
    if (f73 != null) {
      w.writeUint64Value(73, f73);
    }
    // field 74: repeatedStringValue
    final f74 = repeatedStringValue;
    if (f74 != null) {
      for (final e in f74) { w.writeStringValue(74, e); }
    }
    // field 81: optionalDouble
    final f81 = optionalDouble;
    if (f81 != null) {
      w.writeDouble(81, f81);
    }
    // field 82: optionalFloat
    final f82 = optionalFloat;
    if (f82 != null) {
      w.writeFloat(82, f82);
    }
    // field 83: optionalInt32
    final f83 = optionalInt32;
    if (f83 != null) {
      w.writeInt32(83, f83);
    }
    // field 84: optionalInt64
    final f84 = optionalInt64;
    if (f84 != null) {
      w.writeInt64(84, f84);
    }
    // field 85: optionalUint32
    final f85 = optionalUint32;
    if (f85 != null) {
      w.writeUint32(85, f85);
    }
    // field 86: optionalUint64
    final f86 = optionalUint64;
    if (f86 != null) {
      w.writeUint64(86, f86);
    }
    // field 87: optionalSint32
    final f87 = optionalSint32;
    if (f87 != null) {
      w.writeSint32(87, f87);
    }
    // field 88: optionalSint64
    final f88 = optionalSint64;
    if (f88 != null) {
      w.writeSint64(88, f88);
    }
    // field 89: optionalFixed32
    final f89 = optionalFixed32;
    if (f89 != null) {
      w.writeFixed32(89, f89);
    }
    // field 90: optionalFixed64
    final f90 = optionalFixed64;
    if (f90 != null) {
      w.writeFixed64(90, f90);
    }
    // field 91: optionalSfixed32
    final f91 = optionalSfixed32;
    if (f91 != null) {
      w.writeSfixed32(91, f91);
    }
    // field 92: optionalSfixed64
    final f92 = optionalSfixed64;
    if (f92 != null) {
      w.writeSfixed64(92, f92);
    }
    // field 93: optionalBool
    final f93 = optionalBool;
    if (f93 != null) {
      w.writeBool(93, f93);
    }
    // field 94: optionalString
    final f94 = optionalString;
    if (f94 != null) {
      w.writeString(94, f94);
    }
    // field 95: optionalBytes
    final f95 = optionalBytes;
    if (f95 != null) {
      throw UnsupportedError('dart-http wire: field einride.example.syntax.v1.Message.optional_bytes');
    }
    // field 96: optionalEnum
    final f96 = optionalEnum;
    if (f96 != null) {
      w.writeEnum(96, f96.wire); // Enum
    }
    // field 97: optionalMessage
    final f97 = optionalMessage;
    if (f97 != null) {
      final cw = ProtoWireWriter(); f97._writeTo(cw); w.writeRaw(97, cw.toBuffer()); // Message
    }
  }

  factory Message.fromBuffer(List<int> bytes) {
    return Message._readFrom(ProtoWireReader(bytes));
  }

  static Message _readFrom(ProtoWireReader r) {
    final m = Message();
    while (r.next()) {
      switch (r.fieldNumber) {
        // field 1: double
        case 1: {
          m.double_ = r.readDouble();
          break;
        }
        // field 2: float
        case 2: {
          m.float = r.readFloat();
          break;
        }
        // field 3: int32
        case 3: {
          m.int32 = r.readInt32();
          break;
        }
        // field 4: int64
        case 4: {
          m.int64 = r.readInt64();
          break;
        }
        // field 5: uint32
        case 5: {
          m.uint32 = r.readUint32();
          break;
        }
        // field 6: uint64
        case 6: {
          m.uint64 = r.readUint64();
          break;
        }
        // field 7: sint32
        case 7: {
          m.sint32 = r.readSint32();
          break;
        }
        // field 8: sint64
        case 8: {
          m.sint64 = r.readSint64();
          break;
        }
        // field 9: fixed32
        case 9: {
          m.fixed32 = r.readFixed32();
          break;
        }
        // field 10: fixed64
        case 10: {
          m.fixed64 = r.readFixed64();
          break;
        }
        // field 11: sfixed32
        case 11: {
          m.sfixed32 = r.readSfixed32();
          break;
        }
        // field 12: sfixed64
        case 12: {
          m.sfixed64 = r.readSfixed64();
          break;
        }
        // field 13: bool
        case 13: {
          m.bool_ = r.readBool();
          break;
        }
        // field 14: string
        case 14: {
          m.string = r.readString();
          break;
        }
        // field 15: bytes
        case 15: {
          r.skip();
          break;
        }
        // field 16: enum
        case 16: {
          m.enum_ = Enum.fromWire(r.readEnum());
          break;
        }
        // field 17: message
        case 17: {
          m.message = Message._readFrom(r.readNested());
          break;
        }
        // field 18: repeatedDouble
        case 18: {
          m.repeatedDouble = r.readDoubleList();
          break;
        }
        // field 19: repeatedFloat
        case 19: {
          m.repeatedFloat = r.readFloatList();
          break;
        }
        // field 20: repeatedInt32
        case 20: {
          m.repeatedInt32 = r.readVarintList();
          break;
        }
        // field 21: repeatedInt64
        case 21: {
          m.repeatedInt64 = r.readVarintList();
          break;
        }
        // field 22: repeatedUint32
        case 22: {
          m.repeatedUint32 = r.readVarintList();
          break;
        }
        // field 23: repeatedUint64
        case 23: {
          m.repeatedUint64 = r.readVarintList();
          break;
        }
        // field 24: repeatedSint32
        case 24: {
          m.repeatedSint32 = r.readZigzag32List();
          break;
        }
        // field 25: repeatedSint64
        case 25: {
          m.repeatedSint64 = r.readZigzag64List();
          break;
        }
        // field 26: repeatedFixed32
        case 26: {
          m.repeatedFixed32 = r.readFixed32List();
          break;
        }
        // field 27: repeatedFixed64
        case 27: {
          m.repeatedFixed64 = r.readFixed64List();
          break;
        }
        // field 28: repeatedSfixed32
        case 28: {
          m.repeatedSfixed32 = r.readFixed32List();
          break;
        }
        // field 29: repeatedSfixed64
        case 29: {
          m.repeatedSfixed64 = r.readFixed64List();
          break;
        }
        // field 30: repeatedBool
        case 30: {
          m.repeatedBool = r.readBoolList();
          break;
        }
        // field 31: repeatedString
        case 31: {
          m.repeatedString = r.readStringList();
          break;
        }
        // field 32: repeatedBytes
        case 32: {
          m.repeatedBytes = r.readBytesListText();
          break;
        }
        // field 33: repeatedEnum
        case 33: {
          m.repeatedEnum = r.readVarintList().map(Enum.fromWire).toList();
          break;
        }
        // field 34: repeatedMessage
        case 34: {
          m.repeatedMessage = r.readNestedList().map(Message._readFrom).toList();
          break;
        }
        // field 35: mapStringString
        case 35: {
          final er = r.readNested();
          String? k;
          String? v;
          while (er.next()) {
            switch (er.fieldNumber) {
              case 1: {
                k = er.readString();
                break;
              }
              case 2: {
                v = er.readString();
                break;
              }
              default: er.skip();
            }
          }
          if (k != null && v != null) {
            (m.mapStringString ??= {})[k] = v;
          }
          break;
        }
        // field 36: mapStringMessage
        case 36: {
          final er = r.readNested();
          String? k;
          Message? v;
          while (er.next()) {
            switch (er.fieldNumber) {
              case 1: {
                k = er.readString();
                break;
              }
              case 2: {
                v = Message._readFrom(er.readNested());
                break;
              }
              default: er.skip();
            }
          }
          if (k != null && v != null) {
            (m.mapStringMessage ??= {})[k] = v;
          }
          break;
        }
        // field 37: oneofString
        case 37: {
          m.oneofString = r.readString();
          break;
        }
        // field 38: oneofEnum
        case 38: {
          m.oneofEnum = Enum.fromWire(r.readEnum());
          break;
        }
        // field 39: oneofMessage1
        case 39: {
          m.oneofMessage1 = Message._readFrom(r.readNested());
          break;
        }
        // field 40: oneofMessage2
        case 40: {
          m.oneofMessage2 = Message._readFrom(r.readNested());
          break;
        }
        // field 41: any
        case 41: {
          throw UnsupportedError('dart-http wire: google.protobuf.Any not supported');
        }
        // field 42: repeatedAny
        case 42: {
          throw UnsupportedError('dart-http wire: google.protobuf.Any not supported');
        }
        // field 43: duration
        case 43: {
          m.duration = r.readDurationText();
          break;
        }
        // field 44: repeatedDuration
        case 44: {
          m.repeatedDuration = r.readNestedList().map((er) => er.readDurationText()).toList();
          break;
        }
        // field 45: empty
        case 45: {
          m.empty = r.readEmpty();
          break;
        }
        // field 46: repeatedEmpty
        case 46: {
          m.repeatedEmpty = r.readNestedList().map((er) => er.readEmpty()).toList();
          break;
        }
        // field 47: fieldMask
        case 47: {
          m.fieldMask = r.readFieldMaskText();
          break;
        }
        // field 48: repeatedFieldMask
        case 48: {
          m.repeatedFieldMask = r.readNestedList().map((er) => er.readFieldMaskText()).toList();
          break;
        }
        // field 49: struct
        case 49: {
          throw UnsupportedError('dart-http wire: google.protobuf.Struct not supported');
        }
        // field 50: repeatedStruct
        case 50: {
          throw UnsupportedError('dart-http wire: google.protobuf.Struct not supported');
        }
        // field 51: value
        case 51: {
          throw UnsupportedError('dart-http wire: google.protobuf.Value not supported');
        }
        // field 52: repeatedValue
        case 52: {
          throw UnsupportedError('dart-http wire: google.protobuf.Value not supported');
        }
        // field 53: nullValue
        case 53: {
          throw UnsupportedError('dart-http wire: google.protobuf.NullValue not supported');
        }
        // field 54: repeatedNullValue
        case 54: {
          throw UnsupportedError('dart-http wire: google.protobuf.NullValue not supported');
        }
        // field 55: listValue
        case 55: {
          throw UnsupportedError('dart-http wire: google.protobuf.ListValue not supported');
        }
        // field 56: repeatedListValue
        case 56: {
          throw UnsupportedError('dart-http wire: google.protobuf.ListValue not supported');
        }
        // field 57: boolValue
        case 57: {
          m.boolValue = r.readBoolValue();
          break;
        }
        // field 58: repeatedBoolValue
        case 58: {
          m.repeatedBoolValue = r.readNestedList().map((er) => er.readBoolValue()).toList();
          break;
        }
        // field 59: bytesValue
        case 59: {
          m.bytesValue = r.readBytesValueText();
          break;
        }
        // field 60: repeatedBytesValue
        case 60: {
          m.repeatedBytesValue = r.readNestedList().map((er) => er.readBytesValueText()).toList();
          break;
        }
        // field 61: doubleValue
        case 61: {
          m.doubleValue = r.readDoubleValue();
          break;
        }
        // field 62: repeatedDoubleValue
        case 62: {
          m.repeatedDoubleValue = r.readNestedList().map((er) => er.readDoubleValue()).toList();
          break;
        }
        // field 63: floatValue
        case 63: {
          m.floatValue = r.readFloatValue();
          break;
        }
        // field 64: repeatedFloatValue
        case 64: {
          m.repeatedFloatValue = r.readNestedList().map((er) => er.readFloatValue()).toList();
          break;
        }
        // field 65: int32Value
        case 65: {
          m.int32Value = r.readInt32Value();
          break;
        }
        // field 66: repeatedInt32Value
        case 66: {
          m.repeatedInt32Value = r.readNestedList().map((er) => er.readInt32Value()).toList();
          break;
        }
        // field 67: int64Value
        case 67: {
          m.int64Value = r.readInt64Value();
          break;
        }
        // field 68: repeatedInt64Value
        case 68: {
          m.repeatedInt64Value = r.readNestedList().map((er) => er.readInt64Value()).toList();
          break;
        }
        // field 69: uint32Value
        case 69: {
          m.uint32Value = r.readUint32Value();
          break;
        }
        // field 70: repeatedUint32Value
        case 70: {
          m.repeatedUint32Value = r.readNestedList().map((er) => er.readUint32Value()).toList();
          break;
        }
        // field 71: uint64Value
        case 71: {
          m.uint64Value = r.readUint64Value();
          break;
        }
        // field 72: repeatedUint64Value
        case 72: {
          m.repeatedUint64Value = r.readNestedList().map((er) => er.readUint64Value()).toList();
          break;
        }
        // field 73: stringValue
        case 73: {
          m.stringValue = r.readUint64Value();
          break;
        }
        // field 74: repeatedStringValue
        case 74: {
          m.repeatedStringValue = r.readNestedList().map((er) => er.readStringValue()).toList();
          break;
        }
        // field 81: optionalDouble
        case 81: {
          m.optionalDouble = r.readDouble();
          break;
        }
        // field 82: optionalFloat
        case 82: {
          m.optionalFloat = r.readFloat();
          break;
        }
        // field 83: optionalInt32
        case 83: {
          m.optionalInt32 = r.readInt32();
          break;
        }
        // field 84: optionalInt64
        case 84: {
          m.optionalInt64 = r.readInt64();
          break;
        }
        // field 85: optionalUint32
        case 85: {
          m.optionalUint32 = r.readUint32();
          break;
        }
        // field 86: optionalUint64
        case 86: {
          m.optionalUint64 = r.readUint64();
          break;
        }
        // field 87: optionalSint32
        case 87: {
          m.optionalSint32 = r.readSint32();
          break;
        }
        // field 88: optionalSint64
        case 88: {
          m.optionalSint64 = r.readSint64();
          break;
        }
        // field 89: optionalFixed32
        case 89: {
          m.optionalFixed32 = r.readFixed32();
          break;
        }
        // field 90: optionalFixed64
        case 90: {
          m.optionalFixed64 = r.readFixed64();
          break;
        }
        // field 91: optionalSfixed32
        case 91: {
          m.optionalSfixed32 = r.readSfixed32();
          break;
        }
        // field 92: optionalSfixed64
        case 92: {
          m.optionalSfixed64 = r.readSfixed64();
          break;
        }
        // field 93: optionalBool
        case 93: {
          m.optionalBool = r.readBool();
          break;
        }
        // field 94: optionalString
        case 94: {
          m.optionalString = r.readString();
          break;
        }
        // field 95: optionalBytes
        case 95: {
          r.skip();
          break;
        }
        // field 96: optionalEnum
        case 96: {
          m.optionalEnum = Enum.fromWire(r.readEnum());
          break;
        }
        // field 97: optionalMessage
        case 97: {
          m.optionalMessage = Message._readFrom(r.readNested());
          break;
        }
        default: {
          r.skip();
        }
      }
    }
    return m;
  }

}

/// Well-known type: Any
///
/// Maps to Dart: `Map<String, dynamic>`.
///
/// In JSON: `{"@type": xxx, ...}`.

/// Well-known type: Duration
///
/// Maps to Dart: `String` (e.g. `"3.5s"`).

/// Well-known type: Empty
///
/// Maps to Dart: `Map<String, dynamic>`.

/// Well-known type: FieldMask
///
/// Maps to Dart: `String` (comma-separated camelCase paths).

/// Well-known type: Struct
///
/// Maps to Dart: `Map<String, dynamic>`.

/// Well-known type: Value
///
/// Maps to Dart: `dynamic`.

/// Well-known type: NullValue
///
/// Maps to Dart: `String`.

/// Well-known type: ListValue
///
/// Maps to Dart: `List<dynamic>`.

/// Well-known type: BoolValue
///
/// Maps to Dart: `bool`.

/// Well-known type: BytesValue
///
/// Maps to Dart: `String`.

/// Well-known type: DoubleValue
///
/// Maps to Dart: `double`.

/// Well-known type: FloatValue
///
/// Maps to Dart: `double`.

/// Well-known type: Int32Value
///
/// Maps to Dart: `int`.

/// Well-known type: Int64Value
///
/// Maps to Dart: `int`.

/// Well-known type: UInt32Value
///
/// Maps to Dart: `int`.

/// Well-known type: UInt64Value
///
/// Maps to Dart: `int`.

/// Well-known type: StringValue
///
/// Maps to Dart: `String`.

/// NestedMessage
class Message$NestedMessage {
  /// nested_message.string
  String? string;

  Message$NestedMessage({
    this.string,
  });

  factory Message$NestedMessage.fromJson(Map<String, dynamic> json) {
    return Message$NestedMessage(
      string: json['string'] as String?,
    );
  }

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (string != null) json['string'] = string;
    return json;
  }

  @override
  String toString() {
    return 'Message\$NestedMessage(string: $string)';
  }

  @override
  bool operator ==(Object other) =>
    identical(this, other) ||
    other is Message$NestedMessage &&
      runtimeType == other.runtimeType
      && string == other.string
    ;

  @override
  int get hashCode => Object.hashAll([
    string,
  ]);

  Message$NestedMessage copyWith({
    String? string,
  }) {
    return Message$NestedMessage(
      string: string ?? this.string,
    );
  }

  /// 二进制 proto 编码（MQTT 等二进制传输用；字段按编号升序写出）
  Uint8List writeToBuffer() {
    final w = ProtoWireWriter();
    _writeTo(w);
    return w.toBuffer();
  }

  void _writeTo(ProtoWireWriter w) {
    // field 1: string
    final f1 = string;
    if (f1 != null) {
      w.writeString(1, f1);
    }
  }

  factory Message$NestedMessage.fromBuffer(List<int> bytes) {
    return Message$NestedMessage._readFrom(ProtoWireReader(bytes));
  }

  static Message$NestedMessage _readFrom(ProtoWireReader r) {
    final m = Message$NestedMessage();
    while (r.next()) {
      switch (r.fieldNumber) {
        // field 1: string
        case 1: {
          m.string = r.readString();
          break;
        }
        default: {
          r.skip();
        }
      }
    }
    return m;
  }

}

/// NestedEnum
enum Message$NestedEnum {
  nestedenumUnspecified('NESTEDENUM_UNSPECIFIED', 0);

  final String value;
  /// proto 枚举数值（二进制 wire 编解码用，与 JSON 的 value 字符串互补）
  final int wire;
  const Message$NestedEnum(this.value, this.wire);

  static Message$NestedEnum fromString(String v) =>
    values.firstWhere((e) => e.value == v, orElse: () => throw ArgumentError('Unknown Message\$NestedEnum value: ' + v));
  static Message$NestedEnum fromWire(int v) =>
    values.firstWhere((e) => e.wire == v, orElse: () => throw ArgumentError('Unknown Message\$NestedEnum wire value: ' + v.toString()));
  @override
  String toString() => value;
}

class Request {
  Request$Nested? nested;
  List<String>? repeatedString;
  String? string;

  Request({
    this.nested,
    this.repeatedString,
    this.string,
  });

  factory Request.fromJson(Map<String, dynamic> json) {
    return Request(
      nested: json['nested'] != null ? Request$Nested.fromJson(json['nested'] as Map<String, dynamic>) : null,
      repeatedString: (json['repeatedString'] as List<dynamic>?)?.map((e) => e as String).toList(),
      string: json['string'] as String?,
    );
  }

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (nested != null) json['nested'] = nested!.toJson();
    if (repeatedString != null) json['repeatedString'] = repeatedString;
    if (string != null) json['string'] = string;
    return json;
  }

  @override
  String toString() {
    return 'Request(nested: $nested, repeatedString: $repeatedString, string: $string)';
  }

  @override
  bool operator ==(Object other) =>
    identical(this, other) ||
    other is Request &&
      runtimeType == other.runtimeType
      && nested == other.nested
      && repeatedString == other.repeatedString
      && string == other.string
    ;

  @override
  int get hashCode => Object.hashAll([
    nested,
    repeatedString,
    string,
  ]);

  Request copyWith({
    Request$Nested? nested,
    List<String>? repeatedString,
    String? string,
  }) {
    return Request(
      nested: nested ?? this.nested,
      repeatedString: repeatedString ?? this.repeatedString,
      string: string ?? this.string,
    );
  }

  /// 二进制 proto 编码（MQTT 等二进制传输用；字段按编号升序写出）
  Uint8List writeToBuffer() {
    final w = ProtoWireWriter();
    _writeTo(w);
    return w.toBuffer();
  }

  void _writeTo(ProtoWireWriter w) {
    // field 1: string
    final f1 = string;
    if (f1 != null) {
      w.writeString(1, f1);
    }
    // field 2: repeatedString
    final f2 = repeatedString;
    if (f2 != null) {
      w.writeStringList(2, f2);
    }
    // field 3: nested
    final f3 = nested;
    if (f3 != null) {
      final cw = ProtoWireWriter(); f3._writeTo(cw); w.writeRaw(3, cw.toBuffer()); // Request$Nested
    }
  }

  factory Request.fromBuffer(List<int> bytes) {
    return Request._readFrom(ProtoWireReader(bytes));
  }

  static Request _readFrom(ProtoWireReader r) {
    final m = Request();
    while (r.next()) {
      switch (r.fieldNumber) {
        // field 1: string
        case 1: {
          m.string = r.readString();
          break;
        }
        // field 2: repeatedString
        case 2: {
          m.repeatedString = r.readStringList();
          break;
        }
        // field 3: nested
        case 3: {
          m.nested = Request$Nested._readFrom(r.readNested());
          break;
        }
        default: {
          r.skip();
        }
      }
    }
    return m;
  }

}

class Request$Nested {
  String? string;

  Request$Nested({
    this.string,
  });

  factory Request$Nested.fromJson(Map<String, dynamic> json) {
    return Request$Nested(
      string: json['string'] as String?,
    );
  }

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (string != null) json['string'] = string;
    return json;
  }

  @override
  String toString() {
    return 'Request\$Nested(string: $string)';
  }

  @override
  bool operator ==(Object other) =>
    identical(this, other) ||
    other is Request$Nested &&
      runtimeType == other.runtimeType
      && string == other.string
    ;

  @override
  int get hashCode => Object.hashAll([
    string,
  ]);

  Request$Nested copyWith({
    String? string,
  }) {
    return Request$Nested(
      string: string ?? this.string,
    );
  }

  /// 二进制 proto 编码（MQTT 等二进制传输用；字段按编号升序写出）
  Uint8List writeToBuffer() {
    final w = ProtoWireWriter();
    _writeTo(w);
    return w.toBuffer();
  }

  void _writeTo(ProtoWireWriter w) {
    // field 1: string
    final f1 = string;
    if (f1 != null) {
      w.writeString(1, f1);
    }
  }

  factory Request$Nested.fromBuffer(List<int> bytes) {
    return Request$Nested._readFrom(ProtoWireReader(bytes));
  }

  static Request$Nested _readFrom(ProtoWireReader r) {
    final m = Request$Nested();
    while (r.next()) {
      switch (r.fieldNumber) {
        // field 1: string
        case 1: {
          m.string = r.readString();
          break;
        }
        default: {
          r.skip();
        }
      }
    }
    return m;
  }

}

class SyntaxServiceClient {
  final ClientTransport _transport;

  SyntaxServiceClient(this._transport);

  Future<Message> queryOnly(Request request, {Map<String, String>? headers}) async {
    final path = '/v1';
    final queryParams = <String>[];
    if (request.string != null) {
      queryParams.add('string=${Uri.encodeComponent(request.string!.toString())}');
    }
    if (request.repeatedString != null) {
      request.repeatedString!.forEach((x) {
        queryParams.add('repeatedString=${Uri.encodeComponent(x.toString())}');
      });
    }
    if (request.nested?.string != null) {
      queryParams.add('nested.string=${Uri.encodeComponent(request.nested!.string!.toString())}');
    }
    var uri = path;
    if (queryParams.isNotEmpty) {
      uri += '?${queryParams.join("&")}';
    }
    final result = await _transport.unary(uri, 'GET', null, TransportMeta(
      service: 'SyntaxService',
      method: 'QueryOnly',
    ), headers: headers);
    return Message.fromJson(result as Map<String, dynamic>);
  }

  Future<Map<String, dynamic>> emptyVerb(Map<String, dynamic> _request, {Map<String, String>? headers}) async {
    final path = '/v1:emptyVerb';
    final result = await _transport.unary(path, 'GET', null, TransportMeta(
      service: 'SyntaxService',
      method: 'EmptyVerb',
    ), headers: headers);
    return result as Map<String, dynamic>;
  }

  Future<Message> starBody(Request request, {Map<String, String>? headers}) async {
    final path = '/v1:starBody';
    final body = jsonEncode(request.toJson());
    final result = await _transport.unary(path, 'POST', body, TransportMeta(
      service: 'SyntaxService',
      method: 'StarBody',
    ), headers: headers);
    return Message.fromJson(result as Map<String, dynamic>);
  }

  Future<Message> body(Request request, {Map<String, String>? headers}) async {
    final path = '/v1:body';
    final body = jsonEncode(request.nested?.toJson() ?? {});
    final queryParams = <String>[];
    if (request.string != null) {
      queryParams.add('string=${Uri.encodeComponent(request.string!.toString())}');
    }
    if (request.repeatedString != null) {
      request.repeatedString!.forEach((x) {
        queryParams.add('repeatedString=${Uri.encodeComponent(x.toString())}');
      });
    }
    var uri = path;
    if (queryParams.isNotEmpty) {
      uri += '?${queryParams.join("&")}';
    }
    final result = await _transport.unary(uri, 'POST', body, TransportMeta(
      service: 'SyntaxService',
      method: 'Body',
    ), headers: headers);
    return Message.fromJson(result as Map<String, dynamic>);
  }

  Future<Message> path(Request request, {Map<String, String>? headers}) async {
    if (request.string == null) {
      throw ArgumentError('missing required field request.string');
    }
    final path = '/v1/${request.string}:path';
    final queryParams = <String>[];
    if (request.repeatedString != null) {
      request.repeatedString!.forEach((x) {
        queryParams.add('repeatedString=${Uri.encodeComponent(x.toString())}');
      });
    }
    if (request.nested?.string != null) {
      queryParams.add('nested.string=${Uri.encodeComponent(request.nested!.string!.toString())}');
    }
    var uri = path;
    if (queryParams.isNotEmpty) {
      uri += '?${queryParams.join("&")}';
    }
    final result = await _transport.unary(uri, 'POST', null, TransportMeta(
      service: 'SyntaxService',
      method: 'Path',
    ), headers: headers);
    return Message.fromJson(result as Map<String, dynamic>);
  }

  Future<Message> pathBody(Request request, {Map<String, String>? headers}) async {
    if (request.string == null) {
      throw ArgumentError('missing required field request.string');
    }
    final path = '/v1/${request.string}:pathBody';
    final body = jsonEncode(request.nested?.toJson() ?? {});
    final queryParams = <String>[];
    if (request.repeatedString != null) {
      request.repeatedString!.forEach((x) {
        queryParams.add('repeatedString=${Uri.encodeComponent(x.toString())}');
      });
    }
    var uri = path;
    if (queryParams.isNotEmpty) {
      uri += '?${queryParams.join("&")}';
    }
    final result = await _transport.unary(uri, 'POST', body, TransportMeta(
      service: 'SyntaxService',
      method: 'PathBody',
    ), headers: headers);
    return Message.fromJson(result as Map<String, dynamic>);
  }
}

class ApiClient {
  final ClientTransport _transport;

  SyntaxServiceClient? _syntaxService;

  ApiClient(this._transport);

  SyntaxServiceClient get syntaxService {
    _syntaxService ??= SyntaxServiceClient(_transport);
    return _syntaxService!;
  }

  /// Closes all service clients and releases resources.
  void dispose() {
    _syntaxService = null;
  }
}

ApiClient createApiClient(ClientTransport transport) {
  return ApiClient(transport);
}


// @@protoc_insertion_point(dart-http-eof)
