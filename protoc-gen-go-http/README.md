...
生成的代码基于标准库 `net/http`，并通过 [`miwin-plugins`](https://github.com/mimokpl/miwin-plugins) 的 `transport/http/binding` 包完成请求参数绑定、路由注册与统一响应输出。
...
func main() {
    ...
    router := binding.NewRouter(mux) // 使用 miwin-plugins 的 Router
...
请参考上级仓库 [miwin-toolkit](https://github.com/mimokpl/miwin-toolkit) 的许可证信息。
