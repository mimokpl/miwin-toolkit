...
The generated code is built on the standard library `net/http` and uses the [`miwin-plugins`](https://github.com/mimokpl/miwin-plugins) `transport/http/binding` package for request binding, route registration and response writing.
...
```bash
go install github.com/mimokpl/miwin-toolkit/protoc-gen-go-http@latest
```
...
func main() {
    ...
    router := binding.NewRouter(mux) // Use the miwin-toolkit Router
...
Please refer to the parent repository [miwin-toolkit](https://github.com/mimokpl/miwin-toolkit) for license information.
