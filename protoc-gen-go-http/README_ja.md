...
生成されるコードは標準ライブラリ `net/http` に基づいており、[`miwin-plugins`](https://github.com/mimokpl/miwin-plugins) の `transport/http/binding` パッケージを使用してリクエストバインディング、ルーティング登録、レスポンス出力を行います。
...
func main() {
    ...
    router := binding.NewRouter(mux) // miwin-plugins の Router を使用
...
親リポジトリ [miwin-toolkit](https://github.com/mimokpl/miwin-toolkit) のライセンス情報を参照してください。
